...
) : IClaimsTransformation
{
    public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
    {
        ...
        if (principal.HasClaim(c => c.Type == TenantClaimTypes.StaffMemberId))
        {
            return principal;
        }
        ...
        {
            logger.LogWarning(
                "Authenticated principal has no 'sub' claim — cannot resolve a StaffMember record."
            );
            return principal;
        }

        // A staff member's Supabase account is only linked once they've
        // called LinkStaffSupabaseAccountCommand (or, for an Owner, once
        // their StaffMember row is created with SupabaseUserId already
        // set), so this can legitimately find nothing for a brand-new
        // signup — that's not a warning-worthy case.
        var staffMember = await dbContext
            .StaffMembers.AsNoTracking()
            .Where(s => s.SupabaseUserId == supabaseUserId)
            .Select(s => new { s.Id, s.OrganisationId, s.PermissionLevel })
            .FirstOrDefaultAsync();
        ...
        staffIdentity.AddClaim(new Claim(TenantClaimTypes.PermissionLevel, staffMember.PermissionLevel.ToString()));
