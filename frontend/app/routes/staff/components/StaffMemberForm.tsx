    ...
    <section>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            onChange={(e) =>
              set("maxWeeklyHours", Number(e.target.value) || 0)
            }
      ...
      <label className="flex flex-col gap-1.5 mb-3">
        ...
        <select
          ...
          onChange={(e) =>
            set("classification", e.target.value as AwardClassification)
          }
          className="rounded-lg border px-3 py-2 text-sm font-sans font-medium outline-none"
          style={inputStyle}
        >
          {(Object.keys(CLASSIFICATION_META) as AwardClassification[]).map(
            (c) => (
              <option key={c} value={c}>
                {CLASSIFICATION_META[c].label} —{" "}
                {CLASSIFICATION_META[c].description}
              </option>
            ),
          )}
        </select>
